...
# Create a 100% isolated sandbox directory using mktemp -d
TEST_SANDBOX=$(mktemp -d)
MOCK_AGENTS="$TEST_SANDBOX/.agents"

# Copy .agents framework structure to sandbox
cp -r "$SCRIPT_DIR" "$MOCK_AGENTS"

export HOME="$TEST_SANDBOX"
export SKILLS_SRC="$MOCK_AGENTS/skills"
export AGY_SKILLS_DEST="$TEST_SANDBOX/.gemini/config/skills"
export KIRO_SKILLS_DEST="$TEST_SANDBOX/.kiro/steering/skills"
export SNAPSHOT_DIR="$TEST_SANDBOX/snapshots"
...
echo "=== Running Agent Skills 3.0 Comprehensive Isolated Negative & Boundary Tests ==="
echo "Sandbox Directory: $TEST_SANDBOX"

# 1. Test validate.sh on bad skill input
echo "[Negative Test 1] Hardcoded Path & Secret Scanner Failure Check..."
TEST_FAIL_DIR="$MOCK_AGENTS/skills/test_fail_temp"
...
if bash "$MOCK_AGENTS/validate.sh" 2>/dev/null; then
...
TEST_UNREG_DIR="$MOCK_AGENTS/skills/test_unregistered"
mkdir -p "$TEST_UNREG_DIR"
cat << 'EOF' > "$TEST_UNREG_DIR/SKILL.md"
---
...
if bash "$MOCK_AGENTS/adapters/kiro.sh" dry-run does-not-exist 2>/dev/null; then
...
mkdir -p "$MOCK_AGENTS/skills/test_conflict"
cat << 'EOF' > "$MOCK_AGENTS/skills/test_conflict/SKILL.md"
---
name: test_conflict
version: 1.0.0
owner: test
...
for adapter in "$MOCK_AGENTS/adapters/agy.sh" "$MOCK_AGENTS/adapters/kiro.sh"; do
  adapter_name=$(basename "$adapter")
  echo "  Testing $adapter_name conflict fail-closed..."
  
  if [ "$adapter_name" = "agy.sh" ]; then
    dest_path="$AGY_SKILLS_DEST/test_conflict"
  else
    dest_path="$KIRO_SKILLS_DEST/test_conflict"
  fi
  ...
  if bash "$adapter" apply test_conflict --force >/dev/null; then
    if diff -q "$MOCK_AGENTS/skills/test_conflict/SKILL.md" "$dest_path/SKILL.md" >/dev/null; then
...
rm -rf "$MOCK_AGENTS/skills/test_conflict"

# 4. Test Initial Install Rollback
echo "[Negative Test 4] Initial Install Rollback Check..."
mkdir -p "$MOCK_AGENTS/skills/test_initial"
cat << 'EOF' > "$MOCK_AGENTS/skills/test_initial/SKILL.md"
---
name: test_initial
version: 1.0.0
owner: test
...
for adapter in "$MOCK_AGENTS/adapters/agy.sh" "$MOCK_AGENTS/adapters/kiro.sh"; do
  adapter_name=$(basename "$adapter")
  if [ "$adapter_name" = "agy.sh" ]; then
    dest_path="$AGY_SKILLS_DEST/test_initial"
  else
    dest_path="$KIRO_SKILLS_DEST/test_initial"
  fi
...
rm -rf "$MOCK_AGENTS/skills/test_initial"
...
for adapter in "$MOCK_AGENTS/adapters/agy.sh" "$MOCK_AGENTS/adapters/kiro.sh"; do
  adapter_name=$(basename "$adapter")
  if [ "$adapter_name" = "agy.sh" ]; then
    dest_path="$AGY_SKILLS_DEST/test_ssot_del"
  else
    dest_path="$KIRO_SKILLS_DEST/test_ssot_del"
  fi

  # Create initial SSOT skill
  mkdir -p "$MOCK_AGENTS/skills/test_ssot_del"
  cat << 'EOF' > "$MOCK_AGENTS/skills/test_ssot_del/SKILL.md"
---
name: test_ssot_del
version: 1.0.0
owner: test
...
EOF
  ...
  rm -rf "$MOCK_AGENTS/skills/test_ssot_del"
...
echo "=== All 5 Comprehensive Isolated Negative & Boundary Tests Passed Successfully ==="
