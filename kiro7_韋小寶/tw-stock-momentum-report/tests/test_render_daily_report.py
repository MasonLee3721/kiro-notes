...
class RenderTests(unittest.TestCase):
 ...
 def test_writes_machine_readable_report_bundle(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   self.assertTrue(latest.exists());self.assertTrue(dated.exists());self.assertTrue(report.exists());self.assertIn('scores',json.loads(report.read_text()));html=latest.read_text();self.assertIn('\"":"&quot;',html);self.assertNotIn('""":"&quot;',html);self.assertIn("""toggleChart('"+x.stock_code+"')""",html)
