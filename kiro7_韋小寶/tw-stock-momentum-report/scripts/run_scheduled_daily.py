...
def execute(output_dir:Path,attempts:int,interval_seconds:int,publish:bool=False,remote:str="origin")->int:
 ...
 for attempt in range(1,attempts+1):
  ...
  if result.returncode==0:
   atomic_json(status_path,{"status":"completed","completed_at":datetime.now(timezone.utc).isoformat(),"attempts_used":attempt,"log_path":str(log_path)});print(result.stdout.strip())
   if publish:
    publisher=Path(__file__).resolve().parent/"publish_daily_report.py";published=subprocess.run([sys.executable,str(publisher),"--output-dir",str(output_dir),"--remote",remote],text=True,capture_output=True)
    if published.returncode:print(published.stderr.strip(),file=sys.stderr);return 4
...
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--output-dir",type=Path,default=Path("output"));p.add_argument("--attempts",type=int,default=16);p.add_argument("--interval-seconds",type=int,default=900);p.add_argument("--publish",action="store_true");p.add_argument("--remote",default="origin");a=p.parse_args();return execute(a.output_dir,a.attempts,a.interval_seconds,a.publish,a.remote)
