...
def fetch_json(url:str,timeout:float=20,attempts:int=1)->Any:
 error=None
...
def collect(targets_path:Path,end_date:str,days:int,max_months:int=12,cache_dir:Path|None=None,refresh_cache:bool=False)->dict[str,Any]:
 ...
 for target in targets:
  ...
  try:
   ...
   if history[-1]['trade_date']!=end_date:raise PriceHistoryError(f'{market} {code} latest price {history[-1][chr(34)+'trade_date'+chr(34)]}, expected {end_date}')
   outputs.append({'market':market,'stock_code':code,'stock_name':target.get('stock_name'),'trade_date':end_date,'history_count':len(history),'ohlcv_history':history})
...
def main()->int:
 p=argparse.ArgumentParser();p.add_argument('--targets',type=Path,required=True);p.add_argument('--end-date',required=True);p.add_argument('--days',type=int,default=120);p.add_argument('--output',type=Path,required=True);p.add_argument('--cache-dir',type=Path);p.add_argument('--refresh-cache',action='store_true');a=p.parse_args()
 result=collect(a.targets,a.end_date,a.days,cache_dir=a.cache_dir,refresh_cache=a.refresh_cache);a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(f"wrote {len(result['records'])} target price histories of {a.days} days");return 0
